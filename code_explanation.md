...
## 4. Xử lý & Chuẩn hóa dữ liệu (Data Cleaning)
...
### 4.1 Chuẩn hóa Đội bóng & Hợp đồng
```python
...
### 4.2 Chuẩn hóa Cân nặng / Chiều cao
```python
...
### 4.3 Chuẩn hóa Giá trị cầu thủ (Value)
```python
...
```
- Bóc tách ký hiệu Euro `€`, bóc số tiền và tách tiền tố `K` (Nghìn), `M` (Triệu), `B` (Tỷ).
- Tính ra một con số chuẩn theo triệu Euro bằng cách nhân với hệ số. Điều này giúp các con số được hiểu chính xác tỷ lệ độ lớn toán học.

### 4.4 Chuẩn hóa Body Type
```python
...
### 4.5 Áp dụng hàm & Lọc Value > 0
```python
# Gọi các hàm chuẩn hóa để ghi đè vào data
...
```
- Cuối cùng, chúng ta bỏ đi những cầu thủ có giá trị Value = 0 (có thể là cầu thủ dự bị hoặc giải nghệ), và lưu ra file `.csv` mới để chuẩn bị cho quá trình huấn luyện.

---

## 5. Cấp ID duy nhất và phân loại kiểu dữ liệu
...
numeric_columns = ["Age", "Overall rating", "Height", "Weight", "Value", "Total attacking", "Total skill", "Total movement", "Total power", "Total mentality", "Total defending"]
categorical_columns = ["Name", "Nationality", "Team & Contract", "foot", "Best position", "Body type"]
```
**Công việc & Ý nghĩa:**
...
## 6. Phân tích phân phối (Distribution) của các biến dạng số
...
```
**Công việc & Ý nghĩa:**
- Tính toán Logarit (`np.log1p`) cho cột `Value`. Giá trị chuyển nhượng của cầu thủ rất bất cân xứng (vài người siêu sao hàng trăm triệu trong khi hàng ngàn người chỉ vài trăm ngàn). Việc biến đổi log giúp kéo phân phối này bớt "lệch" (skewed), hỗ trợ cho các quá trình chia giỏ dữ liệu (binning) tốt hơn.
- Phía dưới (không trích xuất hoàn toàn), code lặp qua các cột số để vẽ biểu đồ Histogram nhằm giúp chúng ta quan sát rõ hơn về phân bố của cầu thủ theo các chỉ số, chiều cao, độ tuổi...
